# pico_button_message_buzzer.py
# Raspberry Pi Pico + 16x2 I2C LCD (PCF8574+HD44780)
# Reset button on GP18; Piezo buzzer on GP15
# Behavior:
#   - Init: "Press the button"
#   - On press: beep, show "button pressed" for 5s, then return to init message

from machine import Pin, I2C, PWM
import time

# ---- Pins / LCD config (same pinout) ----
I2C_ID       = 0          # I2C0
I2C_SDA_PIN  = 0          # GP0
I2C_SCL_PIN  = 1          # GP1
...
BUZZER_PIN   = 15         # piezo (+) -> GP15, (-) -> GND

DEBOUNCE_MS  = 80
DISPLAY_MS   = 5000       # 5 seconds
...
class Buzzer:
    def __init__(self, pin=BUZZER_PIN, volume=32768):
        self.pwm = PWM(Pin(pin))
        self.pwm.duty_u16(0)
        self.volume = max(0, min(65535, volume))
    def tone(self, freq_hz, dur_ms):
        if freq_hz <= 0:
            self.pwm.duty_u16(0); time.sleep_ms(dur_ms); return
...
class ButtonDisplay:
    ...
    def loop(self):
        while True:
            # handle one-shot press in main loop (and wait for release)
            if self.press_flag:
                self.press_flag = False
                ...
                # Beep on press (A5 ~880 Hz for 150 ms)
                self.buzzer.tone(880, 150)
